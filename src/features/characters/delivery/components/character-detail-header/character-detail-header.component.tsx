...
export const CharacterDetailHeaderComponent: FC<CharacterPageProps> = ({ character }) => {
  ...
  const characterCreated: Character = Character.create(character)

  const onClickHandle = () => {
    updateFavorites(characterCreated)
  }
    ...
    <>
      <header className={styles['detail-header']}>
        <div className={styles['detail-header__wrapper']}>
          ...
          <div className={styles['character-info__wrapper']}>
            <div className={styles['character-info']}>
              <h1 className={styles['character-info__name']}>{character.name}</h1>
              <button className={styles['character-info__fav']} onClick={onClickHandle}>
                {isFavorite(characterCreated) ? (
                  <svg
